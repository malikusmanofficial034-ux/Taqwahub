...
class DuaWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        provideContent {
            val count = currentState(key = CLICK_COUNT_KEY) ?: 0
            val texts = listOf(
                "رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الآخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ", // Arabic
                "Our Lord, give us in this world [that which is] good and in the Hereafter [that which is] good and protect us from the punishment of the Fire.", // English
                "اے ہمارے رب! ہمیں دنیا میں نیکی عطا فرما اور آخرت میں نیکی عطا فرما اور ہمیں آگ کے عذاب سے بچا۔" // Urdu
            )
            ...
            Box(
                modifier = GlanceModifier
                    ...
                    .clickable(actionRunCallback<DuaCycleAction>()),
                ...
                Column(
                    ...
                    Text(
                        text = "🕌 Daily Dua (${labels[currentIndex]})",
                        style = TextStyle(
...
class DuaCycleAction : ActionCallback {
    override suspend fun onAction(
        ...
        DuaWidget().update(context, glanceId)
    }
}

class DuaWidgetReceiver : GlanceAppWidgetReceiver() {
    override val glanceAppWidget: GlanceAppWidget = DuaWidget()
}
